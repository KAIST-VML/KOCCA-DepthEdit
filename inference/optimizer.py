...
class GuideLoss(torch.nn.Module):
    def __init__(self, eps=1e-5):
        super().__init__()
        self._eps = eps

    def forward(self, result, guide):
        norm_result = (result-result.min())/(result.max()-result.min()) * 10

        masked_result = norm_result * (guide != -1)
        masked_guide = guide * (guide != -1)
        loss = torch.sum((masked_result - masked_guide)**2)
...
class BaseOptimizer:
    ...
    def init_click(self, get_prediction, guide, device, shape=None):
        self.best_prediction = None
        self._get_prediction = get_prediction
        self._guide_mask = guide
    ...
    def __call__(self, x):
        ...
        with torch.enable_grad(): 
            ...
            result = self._get_prediction(*opt_vars)

            loss = self.guide_loss(result, self._guide_mask)

        f_val = loss.detach().cpu().numpy()

        if self.best_prediction is None or f_val < self._best_loss:
            self.best_prediction = result.detach()
        ...
        f_grad = opt_params.grad.numpy().ravel().astype(np.float64)
