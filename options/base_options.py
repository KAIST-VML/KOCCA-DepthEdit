...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--use_rgb', action = 'store_true', help='whether source rgb image is given')
        parser.add_argument('--no_label_encoder', action = 'store_true', help = 'whether to encode label along with rgb')
        # for displays
        ...
        # custom options added by JEY
        parser.add_argument('--mobilenet', action = 'store_true', help='whether to use mobilenet backbone')
        parser.add_argument('--train_mode', type=str, default='seg2depth')
        parser.add_argument('--refine_depth', action = 'store_true',help='whether to refine a given depth or not')
        parser.add_argument('--no_guide', action = 'store_true')
        parser.add_argument('--max_stroke', type=int, default=5)
        parser.add_argument('--force_edge', action = 'store_true')
        parser.add_argument('--decoder', type=str, default='default')
        parser.add_argument('--encoder', type=str, default='MobileNetV2')
        parser.add_argument('--segmap_type', type=str, default='')
        ...
        parser.add_argument('--guide_encoder', action = 'store_true')
        parser.add_argument("--experiment", type=str, default = "")
        parser.add_argument("--ckpt", type=str, default = "")
        parser.add_argument("--refine_module", action='store_true')
        parser.add_argument("--local_rank", type=int, default=0)
        parser.add_argument("--guide_empty", type=float, default = -1.0)
        parser.add_argument("--full_skip", action = 'store_true')
        parser.add_argument("--mask_rgb_with_guide", action = 'store_true')

        '''for testing and debugging'''
        parser.add_argument('--debug', action='store_true', help="debug mode. no logging")
        parser.add_argument('--test_decoder', action='store_true', help="use testing decoder")
    ...
    def parse(self, save=False):
        ...
        #if using masked, add one more channel for masked depth(image)
        if opt.mask != '':
            print("Using mask, adding one more channel")
            # opt.semantic_nc += 1

        if opt.use_rgb:
            print("Using RGB image for depth estimation")
            # opt.semantic_nc += 3
        ...
        '''220712'''
        # os.environ['LOCAL_RANK'] = str(opt.gpu_ids[0])
        # os.environ['WORLD_SIZE'] = str(len(opt.gpu_ids))
        ...
        if opt.distributed:
            opt.gpu = "cuda:" + os.environ['LOCAL_RANK']
            print(opt.gpu)
